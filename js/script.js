document.addEventListener('DOMContentLoaded', function () {
    ...
    if (experienceListContainer) {
        ...
        const renderOffices = () => {
            ...
            if (sortBy === 'termurah') {
                filteredOffices.sort((a, b) => parseInt((a.data.price || '0').replace(/[^0-9]/g, '')) - parseInt((b.data.price || '0').replace(/[^0-9]/g, '')));
            } else if (sortBy === 'termahal') {
                filteredOffices.sort((a, b) => parseInt((b.data.price || '0').replace(/[^0-9]/g, '')) - parseInt((a.data.price || '0').replace(/[^0-9]/g, '')));
            }
                        ...
                        <a href="details.html?id=${doc.id}" class="text-decoration-none text-dark">
                            <div class="card border-0 experience-card" data-id="${doc.id}">
                                ...
                                <div class="card-body px-0">
                                    ...
                                    <p class="card-text">Mulai ${item.price || 'N/A'}</p>
                                </div>
                            </div>
                        </a>
                    </div>`;
        ...
        auth.onAuthStateChanged(user => {
            ...
            if (user) {
                ...
                Promise.all([officeQuery, userQuery]).then(([officeSnapshot, userDoc]) => {
                    allOffices = officeSnapshot.docs.map(doc => ({ doc: doc, data: doc.data() }));
                    if (userDoc.exists) {
                ...
                officeQuery.then(officeSnapshot => {
                    allOffices = officeSnapshot.docs.map(doc => ({ doc: doc, data: doc.data() }));
                    currentUserData = { saved: [] }; // Reset saved data
                    populateCities();
                    renderOffices();
                });
            }
            ...
            const loadDetails = async (user) => {
                try {
                    ...
                    if (doc.exists) {
                        ...
                        const dailyPriceNumber = parseInt((item.price || '0').replace(/[^0-9]/g, ''));
                            ...
                            bookingSectionHTML = `
                                ...
                                <div id="price-summary">
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <p id="original-price" class="mb-1 text-muted">${item.price}</p>
                                    </div>
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <h4 id="total-price" data-daily-price="${dailyPriceNumber}">${item.price}</h4>
                                    </div>
                                </div>
                                ...
                                onClose: function (selectedDatesArr) {
                                    if (selectedDatesArr.length === 2) {
                                        ...
                                        if (discount.percentage > 0) {
                                            priceCalculationText.textContent = `Rp${dailyPrice.toLocaleString('id-ID')} x ${duration} hari`;
                                            originalPriceEl.textContent = `Rp${originalTotal.toLocaleString('id-ID')}`;
                                            ...
                                            originalPriceEl.textContent = item.price;
                                        ...
                                        finalBookingDetails = {
                                            ...
                                            totalPrice: `Rp${finalTotal.toLocaleString('id-ID')}`
                                        };
                                    }
                                }
                            });
    ...
    if (addOfficeForm) {
        ...
        const loadOwnerBookings = (ownerId) => {
            ...
            db.collection('bookings').where('ownerId', '==', ownerId).orderBy('bookingTimestamp', 'desc').get().then(async (querySnapshot) => {
                ...
                let bookingsHtml = ''; // 1. Tampung semua HTML di sini dulu
                for (const doc of querySnapshot.docs) {
                    ...
                    // 2. Buat elemen sebagai link <a>, dan simpan ID penyewa (booking.userId) di data-renter-id
                    bookingsHtml += `
                <a href="#" class="list-group-item list-group-item-action contact-renter-link" data-renter-id="${booking.userId}">
                    <div class="d-flex w-100 justify-content-between">
                        <h6 class="mb-1">Pesanan untuk: ${booking.title}</h6>
                        <small>${new Date(booking.bookingTimestamp?.toDate()).toLocaleDateString('id-ID')}</small>
                    </div>
                    <p class="mb-1">Dipesan oleh: <strong>${renterName}</strong></p>
                    <div class="d-flex justify-content-between align-items-center mt-2">
                        <small>Total Bayar: ${booking.totalPrice || booking.price}</small>
                        <span class="badge bg-primary rounded-pill">
                            <i class="bi bi-chat-dots-fill"></i> Hubungi Penyewa
                        </span>
                    </div>
                </a>
            `;
                ...
                ownerBookingsList.innerHTML = bookingsHtml; // 3. Tampilkan semua HTML sekaligus
                ...
                if (error.code === 'failed-precondition') {
                    ...
                    ownerBookingsList.innerHTML = `<div class="alert alert-danger">Gagal memuat pesanan.</div>`;
                }
            });
        };
        auth.onAuthStateChanged(user => {
                    ...
                    Promise.all(uploadPromises)
                        .then(results => {
                            ...
                            const officeData = {
                                ...
                                // SIMPAN SEBAGAI ANGKA (NUMBER), BUKAN TEKS
